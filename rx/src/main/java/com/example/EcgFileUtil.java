...
public class EcgFileUtil {
    ...
    private static EcgFileUtil sInstance;

    private EcgFileUtil(){}

    public static EcgFileUtil getInstance(String path) throws IOException {
        if (sInstance == null) {
            synchronized (EcgFileUtil.class) {
                if (sInstance == null) {
                    sInstance = new EcgFileUtil();
                }
            }
        }
